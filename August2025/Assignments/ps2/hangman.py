    ...
    * After each guess, you should display to the user the 
    ...
    while guesses >= 1 and is_word_guessed(secret_word, letters_guessed) == False:
        ...
        if (not str.isalpha(guess)) or (len(guess) != 1):
            if warnings >= 1:
                warnings -= 1
                print(f"Oops! That is not a letter. You now have {warnings} warning(s) left:",
                      get_guessed_word(secret_word, letters_guessed))
            else:
                guesses -= 1
                print("Oops! That is not a letter. You are out of warnings. You lost a guess",
                      get_guessed_word(secret_word, letters_guessed))

        # already guessed (whether or not it's in the word)
        elif guess in letters_guessed:
            if warnings >= 1:
                warnings -= 1
                print(f"Oops! You've already guessed that letter. You now have {warnings} warning(s) left:",
                      get_guessed_word(secret_word, letters_guessed))
            else:
                guesses -= 1
                print("Oops! You've already guessed that letter. You lost a guess.",
                      get_guessed_word(secret_word, letters_guessed))

        # new wrong guess
        elif guess not in secret_word:
            letters_guessed.append(guess)
            print("Oops! That letter is not in my word.",  # clearer message
                  get_guessed_word(secret_word, letters_guessed))
            if guess in vowels:
                guesses -= 2
            else:
...
def match_with_gaps(my_word, other_word):
    ...
    '''
    # remove spaces in patterns like "t_ _ t"
    pattern = my_word.replace(" ", "")
    
    #check to see if it's the same length
    if len(my_word) != len(other_word):
        return False
    
    # collect revealed letters (letters we can see in pattern)
    revealed = ""
    for char in pattern:
        if char != "_" and char not in revealed:
            revealed = revealed + char
    
        # check each position
    for i in range(len(pattern)):
        if pattern[i] == "_":
            # underscore cannot hide a revealed letter
            if other_word[i] in revealed:
                return False
        else:
            if pattern[i] != other_word[i]:
                return False
